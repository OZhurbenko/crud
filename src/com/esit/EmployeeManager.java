...
public class EmployeeManager {
  ...
  public JSONObject create() {
    ...
    try {
      ...
      //if result > 0, then it means the object was inserted
      if(result > 0) {
        ...
        if(addressId > 0) {
          ...
          //execute create Employee query here and get the result
          result = preparedStatement.executeUpdate();

          //couldn't insert into the Employee table
          if(result < 1) {
            return new JSONObject().put("error", 500);
          }
          //coultn't get the id of the newly created Address object
        } else {
          return new JSONObject().put("error", 500);
        }
        //coultn'd insert into the Address table
      } else {
        return new JSONObject().put("error", 500);
      }
    } catch (Exception e) {
  ...
  public JSONObject getAll() throws NamingException {
    ...
    try {
      ...
      if(resultSet.isBeforeFirst()) {
        ...
        //return 204 if there is no results
      } else {
  ...
  public JSONObject getEmployeeById(int id) throws NamingException {
    ...
    try {
      ...
      if(resultSet.isBeforeFirst()) {
        ...
        //adding employee info to the final json object
  ...
  public JSONObject updateStatus(int employeeId) {
    ...
    try {
      employeeObj = this.getEmployeeById(employeeId);
      if(employeeObj.length() > 0 && !employeeObj.has("error")) {
