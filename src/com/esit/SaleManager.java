...
public class SaleManager {
  ...
  public JSONObject create() {
    ...
    int programType = 0;
    int saleId = 0;
    JSONObject customerIdJson = new JSONObject();
    ...
    try {
      ...
      customerIdJson = customer.create(conn);

      //if returned id > 0 then everything went well
      if(customerIdJson.has("customerId")) {
          ...
          if(salesRepId > 0 && programType > 0) {
            ...
            preparedStatement.setInt(1, customerIdJson.getInt("customerId"));
            ...
            if(result > 0) {
              //fetching the id of the sale we just created
              preparedStatement = dbConnection.prepareStatement(getSaleQuery);
              preparedStatement.setInt(1, customerIdJson.getInt("customerId"));
              preparedStatement.setInt(2, Integer.parseInt(this.getSalesRepId()));
              preparedStatement.setInt(3, Integer.parseInt(this.getProgramType()));
            ...
            //failed to validate salesRepId or propgramType, returning an error
          } else {
            return jsonObject.put("error", 500);
          }

        //failed to create a new customer and retrieve the customerId back
      } else {
    ...
    return jsonObject.put("saleId", saleId);
  }

  public JSONObject putEnvelopeId(int id, MultivaluedMap<String, String> formParams) {
    int result = 0;
    Connection dbConnection = null;
    PreparedStatement preparedStatement = null;
  ...
  public JSONObject setFolderId(int id, MultivaluedMap<String, String> formParams) {
    Connection dbConnection = null;
    PreparedStatement preparedStatement = null;
    ...
    try {
      ...
      if(result >= 0) {
        ...
        if(resultSet.next()) {
          if (resultSet.getString("folderId").equals(folderId)) {
            jsonObject.put("saleId", Integer.parseInt(resultSet.getString("saleId")));
          }
        }
      } else {
